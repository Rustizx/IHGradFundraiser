...
const createCheckoutSession = async (req, res) => {
  ...
  try {
    ...
    stripe.balance.retrieve(async (err, balance) => {
      if ( amount > 500 && amount < (550000-((balance.pending[0].amount)*1.04)) && !err ) {
        ...
        res.status(400).json({ error: "amount", amount: Math.floor((550000-((balance.pending[0].amount)*1.04)) / 100)});
      }
    });
  } catch (e) {
